create or replace Procedure Insert_Art_Cotizado_List
...
BEGIN
  ...
  --Cant Correct About Family
   ...
   insert into articulosCotizados_obj values (cot_id,art_id,cant,precioxCant);
   
   --Inserto en lista de cotizacion el articulo recien insertado
   insert into table(
            select lista_art_cotizados  from cotizaciones_obj where codigo = cot_id  )
             values 
             (
             (SELECT REF(f) FROM articulosCotizados_OBJ f WHERE f.codigo = cot_id and f.codigoArticulo = art_id)
             );
